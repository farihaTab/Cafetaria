...
class States:
    def __init__(self,hotfoodServer, sandwitchServer, cashier):
        ...
        # States
        self.cashierQ = [ [] for i in range(cashier) ]
        self.hotfoodQ = [ [] for i in range(hotfoodServer)]
        self.sandwichQ = [ [] for i in range(sandwitchServer)]
...
def experiment1():
    seed = 101
    sim = Simulator(seed)
    sim.configure(Params(1/30, 1, 1, 2), States())
    sim.run()
    sim.printResults()
